...
describe("relayed push accounts", () => {
  it("names the owning account on every dispatch", async () => {
    ...
    try {
      db.prepare("INSERT INTO subscriptions (account_id,device_id,topic_hash) VALUES ('acc_1','dev_1',?)").run(remoteHash);
  ...
  it("dispatches once per subscribed account, each carrying its own", async () => {
    ...
    try {
      db.prepare("INSERT INTO subscriptions (account_id,device_id,topic_hash) VALUES ('acc_1','dev_1',?)").run(remoteHash);
      db.prepare("INSERT INTO subscriptions (account_id,device_id,topic_hash) VALUES ('acc_2','dev_2',?)").run(remoteHash);
  ...
  it("sends nothing and logs for a hash this relay serves a topic for", async () => {
    ...
    try {
      ...
      db.prepare("INSERT INTO subscriptions (account_id,device_id,topic_hash) VALUES ('acc_1','dev_1',?)").run(localHash);
