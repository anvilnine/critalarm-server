...
export function addToken(db: Database.Database, clock: Clock, accountId: string, name: string) { const topic = ownedTopic(db, accountId, name); if (topic === undefined) return undefined; const token = `tk_${randomUUID().replaceAll("-", "")}`; db.prepare("INSERT INTO topic_tokens (id,topic_id,hash,created_at) VALUES (?,?,?,?)").run(`tok_${randomUUID()}`,topic.id,createHash("sha256").update(token).digest("hex"),clock.now()); return { token }; }
export function deleteToken(db: Database.Database, accountId: string, name: string, token: string): "deleted" | "final" | "missing" { const topic=ownedTopic(db,accountId,name); if(topic===undefined)return "missing"; return db.transaction(()=>{const row=db.prepare("SELECT id FROM topic_tokens WHERE topic_id=? AND hash=?").get(topic.id,createHash("sha256").update(token).digest("hex"));if(row===undefined)return "missing";const count=(db.prepare("SELECT COUNT(*) AS count FROM topic_tokens WHERE topic_id=?").get(topic.id) as {count:number}).count;if(count===1)return "final";db.prepare("DELETE FROM topic_tokens WHERE id=?").run((row as {id:string}).id);return "deleted";})(); }
