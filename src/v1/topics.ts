...
export function deleteToken(db: Database.Database, accountId: string, name: string, tokenId: string) { const topic=ownedTopic(db,accountId,name); if(topic===undefined)return false; return db.prepare("DELETE FROM topic_tokens WHERE id=? AND topic_id=?").run(tokenId,topic.id).changes>0; }
