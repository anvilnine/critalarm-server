...
export function createTopic(db: Database.Database, config: Config, clock: Clock, accountId: string, name: string, critical = false) {
...
export function listTokens(db: Database.Database, accountId: string, name: string) { const topic=ownedTopic(db,accountId,name); if(topic===undefined)return undefined; return db.prepare("SELECT id AS token_id, created_at FROM topic_tokens WHERE topic_id=? ORDER BY created_at, id").all(topic.id) as {token_id:string;created_at:number}[]; }
