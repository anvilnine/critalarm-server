...
describe("incident list filters", () => {
  it("applies state and topic filters before limiting newest incidents", async () => {
    ...
    for(const [id,topic,state,opened] of [["inc_open","prod","open",40],["inc_acked","acked","acked",30],["inc_closed","prod","closed",20],["inc_stage","stage","open",10]]) db.prepare("INSERT INTO incidents (id,topic_id,state,opened_at,last_message_at,updated_at,max_ring_s) VALUES (?,?,?,?,?,?,60)").run(id,topic,state,opened,opened,opened);
...
describe("incident list limit", () => {
  it("defaults to 20, clamps above 200, and still rejects a limit below 1", async () => {
    ...
    const insert=db.prepare("INSERT INTO incidents (id,topic_id,state,opened_at,last_message_at,updated_at,max_ring_s) VALUES (?,'t','closed',?,?,?,60)"); for(let n=1;n<=201;n+=1) insert.run(`inc_${n}`,n,n,n);
    const app=createApp({config:{baseUrl:"https://a",relayUrl:"https://r",relayContent:"none",listen:":8080",port:8080,dataDir:"/data",behindProxy:false},db,clock:{now:()=>1},ids:{message:()=>"m",incident:()=>"i",timer:()=>"t"},dispatch:async()=>{}});const headers={Authorization:"Bearer dv_a"};
    const ids=async (path:string)=>((await (await app.request(path,{headers})).json()) as {id:string}[]).map(value=>value.id);
...
describe("incident list since", () => {
  it("returns only incidents opened after the second given, alongside the other filters", async () => {
    ...
    for(const [id,topic,opened] of [["inc_old","prod",100],["inc_edge","prod",200],["inc_new","prod",300],["inc_stage","stage",400]]) db.prepare("INSERT INTO incidents (id,topic_id,state,opened_at,last_message_at,updated_at,max_ring_s) VALUES (?,?,'closed',?,?,?,60)").run(id,topic,opened,opened,opened);
...
describe("incident list history window", () => {
  ...
  function windowSetup(mode: "relay" | "hosted", tier: "free" | "hosted") {
    ...
    db.prepare("INSERT INTO topics (id,account_id,name,base_url,topic_hash,critical,repeat_interval_s,max_ring_s,desk_timer_s,relay_content,created_at) VALUES ('t','a','prod','https://a','h',1,30,60,30,'none',1)").run();
    for(const [id,ageDays] of [["inc_day6",6],["inc_day8",8],["inc_day89",89]] as const) db.prepare("INSERT INTO incidents (id,topic_id,state,opened_at,last_message_at,updated_at,max_ring_s) VALUES (?,'t','closed',?,?,?,60)").run(id,now-ageDays*day,now-ageDays*day,now-ageDays*day);
