...
function setup(){const db=openDatabase(":memory:");migrate(db);for(const id of ["a","b"])db.prepare("INSERT INTO accounts (id,tier,created_at) VALUES (?, 'free',1)").run(id);for(const [id,a,t] of [["da","a","dv_a"],["db","b","dv_b"]])db.prepare("INSERT INTO devices (id,account_id,device_token_hash,platform,push_token,last_seen) VALUES (?, ?, ?, 'ios','x',1)").run(id,a,createHash("sha256").update(t).digest("hex"));const ids={message:()=>"m_1",incident:()=>"inc_1",timer:()=>"tm_1"};return createApp({config:{baseUrl:"https://alerts.example.com",relayUrl:"https://relay.critalarm.app",relayContent:"none",listen:":8080",port:8080,dataDir:"/data",behindProxy:false},db,clock:{now:()=>1000},ids,dispatch:async()=>{}})}
